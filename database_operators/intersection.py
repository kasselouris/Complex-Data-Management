	...
	while True:
		...
		if(R_row[0] == S_row[0]):
			if(R_row == S_row):	# or R_row[1] == S_row[1] its the same thing
				if((R_row != previous_R_row) and (S_row != previous_S_row)):
					output.writerow(R_row) # or S_row its the same thing
			...
			if((R_row != previous_R_row) and (R_row == previous_S_row)):
				output.writerow(R_row) #or previous_S_row its the same thing
			...
			if((S_row != previous_S_row) and (S_row == previous_R_row)):
				output.writerow(S_row) #or previous_R_row its the same thing
...
def main(args):
	if(len(args) != 3):
		...
		print("Try: python3 intersection.py R_sorted.tsv S_sorted.tsv")
		sys.exit(1)

	R_sorted_fd = open(args[1], "r")	
	S_sorted_fd = open(args[2], "r")

	with open('RintersectionS.tsv', 'w') as out_file:
		tsv_writer = csv.writer(out_file, delimiter='\t')

		intersection(R_sorted_fd, S_sorted_fd, tsv_writer)
