...
def main(args):
	if(len(args) != 3):
		...
		print("Try: python3 union.py R_sorted.tsv S_sorted.tsv")
		sys.exit(1)

	R_sorted_fd = open(args[1], "r")	
	S_sorted_fd = open(args[2], "r")

	with open('RunionS.tsv', 'w') as out_file:
		tsv_writer = csv.writer(out_file, delimiter='\t')

		union(R_sorted_fd, S_sorted_fd, tsv_writer)
